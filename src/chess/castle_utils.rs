use super::{
    ...
    board::{Square, Castle}, color::Color,
};
...
impl CastleUtility {
    ...
    fn get_castle_masks() -> (u64, u64, u64, u64, u64, u64) {
        ...
        let short_castle_origin = mask(CastleUtility::KING_ORIGIN) | mask(CastleUtility::SHORT_ROOK_ORIGIN);
        let long_castle_origin = mask(CastleUtility::KING_ORIGIN) | mask(CastleUtility::LONG_ROOK_ORIGIN);
    ...
    pub fn get_rook_origin(castle : Castle, color : Color) -> usize {
        match (color, castle) {
            ...
            (Color::White, Castle::Short) => CastleUtility::SHORT_ROOK_ORIGIN + BOARD_SIZE as usize * 7, 
            (Color::White, Castle::Long) => CastleUtility::LONG_ROOK_ORIGIN + BOARD_SIZE as usize * 7,
        }
    }
    ...
    pub fn get_king_landing(castle : Castle, color : Color) -> usize {
        match (color, castle) {
            ...
            // (truncated bits in this case are always 1, 
            // and we indeed want to fill with 1 at the beginning of the integer)
            (Color::White, Castle::Short) => CastleUtility::SHORT_KING_LANDING + BOARD_SIZE as usize * 7, 
            (Color::White, Castle::Long) => CastleUtility::LONG_KING_LANDING + BOARD_SIZE as usize * 7,
        }
    }
    ...
    pub fn get_castle_origin(&self, castle : Castle, color : Color)->u64 {
        match (color, castle) {
